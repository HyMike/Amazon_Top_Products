...
_resolver: _core.RunVar[HostnameResolver |
                        None] = _core.RunVar("hostname_resolver")
_socket_factory: _core.RunVar[SocketFactory |
                              None] = _core.RunVar("socket_factory")
...
# It would be possible to @overload the return value depending on
# Literal[AddressFamily.INET/6], but should probably be added in typeshed
# first
async def getaddrinfo(
    ...
    tuple[
        AddressFamily, SocketKind, int, str, tuple[str,
...
def _sniff_sockopts_for_fileno(
    ...
    # and then we'll throw it away and construct a new one with the correct
    # metadata.
    ...
    try:
        ...
        # Unwrap it again, so that sockobj.__del__ doesn't try to close our
        # socket
        sockobj.detach()
    return family, type, proto
...
class SocketType:
    ...
    # argument names with __ used because of typeshed, see comment for recv in
    # _SocketType
    ...
    # return type of socket.socket.recvfrom_into in typeshed is tuple[bytes,
    # Any]
    def recvfrom_into(
        __self, buffer: Buffer, nbytes: int = 0, flags: int = 0
...
class _SocketType(SocketType):
    def __init__(self, sock: _stdlib_socket.socket):
        if not isinstance(sock, _stdlib_socket.socket):
    ...
    if TYPE_CHECKING:
        # return type of socket.socket.recvfrom in typeshed is tuple[bytes,
        # Any]
        def recvfrom(
            __self, __bufsize: int, __flags: int = 0
        ) -> Awaitable[tuple[bytes, AddressFormat]]:
    ...
    if TYPE_CHECKING:
        # return type of socket.socket.recvfrom_into in typeshed is
        # tuple[bytes, Any]
        def recvfrom_into(
            __self, buffer: Buffer, nbytes: int = 0, flags: int = 0
        ) -> Awaitable[tuple[int, AddressFormat]]:
    ...
    @_wraps(_stdlib_socket.socket.sendto, assigned=(),
            updated=())  # type: ignore[misc]
    async def sendto(self, *args: Any) -> int:
