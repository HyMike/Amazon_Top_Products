...
async def test_handshake_over_terrible_network(
    ...
    async with dtls_echo_server() as (_, address):
        async with trio.open_nursery() as nursery:
            ...
            # type: ignore[assignment]  # TODO: Fix FakeNet typing
        ...
        def route_packet(packet: UDPPacket) -> None:
            try:
                ...
                if len(cookie) != 0:
                    ...
                    # let's corrupt the next offset so the handshake should
                    # fail
                    payload = bytearray(packet.payload)
...
async def test_openssl_retransmit_doesnt_break_stuff() -> None:
    ...
    # type: ignore[assignment]  # TODO add type annotations for FakeNet
    fn.route_packet = route_packet
...
async def test_handshake_handles_minimum_network_mtu(
    ...
    # Fake network that has the minimum allowable MTU for whatever protocol
    # we're using.
    fn = FakeNet()
    fn.enable()
