...
class MonkeypatchedGAI:
    def __init__(
            self, orig_getaddrinfo: Callable[..., GetAddrInfoResponse]) -> None:
    ...
    def getaddrinfo(self, *args: Any, **
                    kwargs: Any) -> GetAddrInfoResponse | str:
...
async def test_SocketType_resolve(socket_type: AddressFamily, addrs: Addresses) -> None:
    ...
    with tsocket.socket(family=socket_type) as sock:
        # testing internal functionality, so we check it against the internal
        # type
        ...
        # TODO: this implies that we can send host=None, but what does that
        # imply for the return value, and other stuff?
        for null in [None, ""]:
        ...
        for local in [False, True]:
            ...
            with pytest.raises(  # noqa: PT012
                ...
                if v6:
                    ...
                    # I guess in theory there could be enough overloads that
                    # this could error?
...
async def test_resolve_address_exception_in_connect_closes_socket() -> None:
    # Here we are testing issue 247, any cancellation will leave the socket
    # closed
    with _core.CancelScope() as cancel_scope:
        with tsocket.socket() as sock:
...
async def test_custom_hostname_resolver(monkeygai: MonkeypatchedGAI) -> None:
    ...
    assert tsocket.set_custom_hostname_resolver(
        cr) is None  # type: ignore[arg-type]
...
async def test_custom_socket_factory() -> None:
    ...
    assert tsocket.set_custom_socket_factory(
        csf) is None  # type: ignore[arg-type]
...
@pytest.mark.skipif(not hasattr(tsocket, "AF_UNIX"),
                    reason="no unix domain sockets")
