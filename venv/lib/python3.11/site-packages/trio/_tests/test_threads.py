...
async def test_run_in_worker_thread_fail_to_spawn(
    ...
    monkeypatch.setattr(
        _core._thread_cache.ThreadCache,
        "start_thread_soon",
        bad_start)
...
def test_from_thread_run_during_shutdown() -> None:
    ...
    async def main(use_system_task: bool) -> None:
        save.append(agen(_core.current_trio_token()
                    if use_system_task else None))
...
async def test_recursive_to_thread() -> None:
    ...
    def get_tid_then_reenter() -> int:
        ...
        # The nesting of wrapper functions loses the return value of
        # threading.get_ident
...
async def test_reentry_doesnt_deadlock() -> None:
    ...
    # XXX consider running this in a subprocess with a timeout, if it comes up
    # again!
