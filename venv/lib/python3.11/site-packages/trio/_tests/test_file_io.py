...
def test_dir_matches_wrapped(
    ...
    assert all(attr in dir(async_file)
               for attr in attrs if attr in dir(wrapped))
...
def test_type_stubs_match_lists() -> None:
    ...
    # Compare two lists so that we can easily see duplicates, and see what is
    # different overall.
...
def test_async_methods_generated_once(
        async_file: AsyncIOWrapper[mock.Mock]) -> None:
