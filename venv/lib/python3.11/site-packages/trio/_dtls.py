...
def records_untrusted(packet: bytes) -> Iterator[Record]:
    ...
    while i < len(packet):
        ...
        # have passed is_client_hello_untrusted, which filters out short
        # packets)
    ...
    def encode_volley(
        ...
        for message in messages:
            if isinstance(message, OpaqueHandshakeMessage):
                ...
                space = mtu - len(packet) - \
                    RECORD_HEADER.size - len(message.payload)
                if space <= 0:
                    packets.append(packet)
                    packet = bytearray()
...
def _make_cookie(
    ...
    return (salt + hmac.digest(key, signable_data,
            COOKIE_HASH))[:COOKIE_LENGTH]
    ...
    if not valid_cookie(endpoint._listening_key, cookie, address, bits):
        ...
        if old_stream is not None:
            if old_stream._client_hello == (cookie, bits):
                ...
                # Ok, this *really is* a new handshake; the old stream should
                # go away.
...
class DTLSChannel(trio.abc.Channel[bytes], metaclass=NoPublicConstructor):
    ...
    # to handle receiving it properly though, which might be easier if we send
    # it...
                    ...
                    async for packet in self._q.r:
                        ...
                        if maybe_volley:
                            if (
                                ...
                                # time the value may be reset to the initial
                                # value."
                ...
                if cscope.cancelled_caught:
                    ...
                    if volley_failed_sends == 2:
                        ...
                        self._handshake_mtu = min(
                            self._handshake_mtu, worst_case_mtu(
...
class DTLSEndpoint:
    ...
    def __init__(
        ...
        *,
        ...
        self._streams: WeakValueDictionary[Any,
                                           DTLSChannel] = WeakValueDictionary()
