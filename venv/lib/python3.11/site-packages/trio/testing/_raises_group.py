...
class _ExceptionInfo(Generic[E]):
    ...
    def fill_unfilled(
            self, exc_info: tuple[type[E], E, types.TracebackType]) -> None:
    ...
    def getrepr(
        ...
        tbfilter: bool | Callable[[
            _ExceptionInfo[BaseException]], Traceback] = True,
        funcargs: bool = False,
...
# being a type error to do `with RaisesGroup(ValueError()): ...` - but
# that will error on runtime.

# We lie to type checkers that we inherit, so excinfo.value and
# sub-exceptiongroups can be treated as ExceptionGroups
if TYPE_CHECKING:
    SuperClass = BaseExceptionGroup
# Inheriting at runtime leads to a series of TypeErrors, so we do not want
# to do that.
else:
    SuperClass = Generic
...
class RaisesGroup(
    ...
    def __enter__(self) -> ExceptionInfo[BaseExceptionGroup[E]]:
        self.excinfo: ExceptionInfo[BaseExceptionGroup[E]
        ...
        for e in actual_exceptions:
            for rem_e in remaining_exceptions:
                if (
                    ...
                    remaining_exceptions.remove(
                        rem_e)  # type: ignore[arg-type]
                    break
            else:
                return False
        return True
