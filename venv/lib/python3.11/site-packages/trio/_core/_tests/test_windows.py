...
def test_winerror(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # With an explicit number passed in, it overrides what getwinerror()
    # returns.
    with pytest.raises(
...
) -> Generator[tuple[BufferedWriter, int], None, None]:
    import msvcrt
...
def test_lsp_that_hooks_select_gives_good_error(
    ...
    monkeypatch.setattr(
        _io_windows,
        "_get_underlying_socket",
        patched_get_underlying)
    with pytest.raises(
