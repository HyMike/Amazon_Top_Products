...
class Deadlines:
    ...
    def expire(self, now: float) -> bool:
        ...
        if len(self._heap) > self._active * 2 + \
                DEADLINE_HEAP_MIN_PRUNE_THRESHOLD:
            self._prune()
...
class CancelStatus:
    ...
    abandoned_by_misnesting: bool = attr.ib(
...
class Nursery(metaclass=NoPublicConstructor):
    ...
    def _check_nursery_closed(self) -> None:
        if not any([self._nested_child_running,
                   self._children, self._pending_starts]):
            self._closed = True
    ...
    async def _nested_child_finished(
        ...
        # and there is just one contained exception) if there are pending
        # exceptions
        ...
        if self._pending_excs:
            try:
                if not self._strict_exception_groups and len(
                        self._pending_excs) == 1:
...
class Task(metaclass=NoPublicConstructor):
    ...
    _abort_func: Callable[[_core.RaiseCancelT],
                          Abort] | None = attr.ib(default=None)
        ...
        while coro is not None:
            if hasattr(coro, "cr_frame"):
                ...
                for referent in gc.get_referents(coro):
                    ...
                    # either cpython changed or we are running on an
    ...
    def _activate_cancel_status(
            self, cancel_status: CancelStatus | None) -> None:
        if self._cancel_status is not None:
    ...
    def _attempt_abort(self, raise_cancel: _core.RaiseCancelT) -> None:
        ...
        if not isinstance(success, Abort):
...
class GuestState:
    ...
    run_sync_soon_threadsafe: Callable[[
        Callable[[], object]], object] = attr.ib()
    run_sync_soon_not_threadsafe: Callable[[
        Callable[[], object]], object] = attr.ib()
    ...
    def guest_tick(self) -> None:
        ...
        if timeout <= 0 or isinstance(
                events_outcome, Error) or events_outcome.value:
...
def start_guest_run(
    ...
    *done_callback* at some point after :func:`start_guest_run` returns
    ...
    for _tick in range(
            5):  # expected need is 2 iterations + leave some wiggle room
