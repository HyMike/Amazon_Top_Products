    ...
    def __exit__(self, ty, value, tb):
        ...
        if _TRIO_MULTI_ERROR:  # pragma: no cover
            filtered_exception = trio.MultiError.filter(
                _ignore_cancel, value)  # pylint: disable=no-member
    ...
    async def _aclose(self, code, reason):
        ...
        try:
            if self._wsproto.state == ConnectionState.OPEN:
                ...
                # event to peer, while setting the local close reason to
                # normal.
...
class WebSocketServer:
    ...
    def port(self):
        ...
        try:
            ...
            raise RuntimeError(
                f'This socket does not have a port: {repr(listener)}') from None
