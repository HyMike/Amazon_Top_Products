...
class Connection:
    ...
    def _server_switch_event(self, event: Event) -> Optional[Type[Sentinel]]:
        if isinstance(
                event, InformationalResponse) and event.status_code == 101:
    ...
    def _process_event(self, role: Type[Sentinel], event: Event) -> None:
        ...
        # succeeds.
        old_states = dict(self._cstate.states)
        if role is CLIENT and isinstance(event, Request):
        ...
        # 100-continue
        if isinstance(event, Request) and has_expect_100_continue(event):
