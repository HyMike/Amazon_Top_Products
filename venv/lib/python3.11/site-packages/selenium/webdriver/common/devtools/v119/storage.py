...
class InterestGroupDetails:
    ...
    def from_json(cls, json):
        return cls(
            ...
            name=str(json['name']),
            expiration_time=network.TimeSinceEpoch.from_json(
                json['expirationTime']),
            ...
            ad_components=[InterestGroupAd.from_json(
                i) for i in json['adComponents']],
            bidding_logic_url=str(
                json['biddingLogicURL']) if 'biddingLogicURL' in json else None,
            bidding_wasm_helper_url=str(
                json['biddingWasmHelperURL']) if 'biddingWasmHelperURL' in json else None,
            update_url=str(json['updateURL']) if 'updateURL' in json else None,
            trusted_bidding_signals_url=str(
                json['trustedBiddingSignalsURL']) if 'trustedBiddingSignalsURL' in json else None,
            user_bidding_signals=str(
                json['userBiddingSignals']) if 'userBiddingSignals' in json else None,
        )
...
class SharedStorageUrlWithMetadata:
    ...
    def to_json(self):
        json = dict()
        json['url'] = self.url
        json['reportingMetadata'] = [i.to_json()
                                     for i in self.reporting_metadata]
        return json
    ...
    def from_json(cls, json):
        return cls(
            url=str(json['url']),
            reporting_metadata=[SharedStorageReportingMetadata.from_json(
                i) for i in json['reportingMetadata']],
        )
...
class SharedStorageAccessParams:
    ...
    def to_json(self):
        ...
        if self.urls_with_metadata is not None:
            json['urlsWithMetadata'] = [i.to_json()
                                        for i in self.urls_with_metadata]
    ...
    def from_json(cls, json):
        return cls(
            script_source_url=str(
                json['scriptSourceUrl']) if 'scriptSourceUrl' in json else None,
            operation_name=str(
                json['operationName']) if 'operationName' in json else None,
            serialized_data=str(
                json['serializedData']) if 'serializedData' in json else None,
            urls_with_metadata=[SharedStorageUrlWithMetadata.from_json(
                i) for i in json['urlsWithMetadata']] if 'urlsWithMetadata' in json else None,
            key=str(json['key']) if 'key' in json else None,
            value=str(json['value']) if 'value' in json else None,
            ignore_if_present=bool(
                json['ignoreIfPresent']) if 'ignoreIfPresent' in json else None,
        )
...
class AttributionReportingSourceRegistration:
    ...
    def from_json(cls, json):
        return cls(
            ...
            filter_data=[AttributionReportingFilterDataEntry.from_json(
                i) for i in json['filterData']],
            aggregation_keys=[AttributionReportingAggregationKeysEntry.from_json(
                i) for i in json['aggregationKeys']],
            expiry=int(json['expiry']) if 'expiry' in json else None,
            event_report_window=int(
                json['eventReportWindow']) if 'eventReportWindow' in json else None,
            event_report_windows=AttributionReportingEventReportWindows.from_json(
                json['eventReportWindows']) if 'eventReportWindows' in json else None,
            aggregatable_report_window=int(
                json['aggregatableReportWindow']) if 'aggregatableReportWindow' in json else None,
            debug_key=UnsignedInt64AsBase10.from_json(
                json['debugKey']) if 'debugKey' in json else None,
        )
...
class AttributionReportingSourceRegistered:
    ...
    **EXPERIMENTAL**
    ...
    @classmethod
    def from_json(
            cls, json: T_JSON_DICT) -> AttributionReportingSourceRegistered:
        return cls(
