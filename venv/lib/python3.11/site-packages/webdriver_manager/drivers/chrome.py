...
class ChromeDriver(Driver):
    ...
    def get_driver_download_url(self, os_type):
        ...
        # by default, for lower versions we replace "mac_arm64" to old format -
        # "mac64_m1".
        if version.parse(driver_version_to_download) < version.parse(
                "106.0.5249.61"):
    ...
    def get_latest_release_version(self):
        ...
        if determined_browser_version is not None and version.parse(
                determined_browser_version) >= version.parse("115"):
            url = "https://googlechromelabs.github.io/chrome-for-testing/latest-patch-versions-per-build.json"
            response = self._http_client.get(url)
            response_dict = json.loads(response.text)
            determined_browser_version = response_dict.get(
                "builds").get(determined_browser_version).get("version")
            return determined_browser_version
        # Remove the build version (the last segment) from
        # determined_browser_version for version < 113
    ...
    def get_url_for_version_and_platform(self, browser_version, platform):
        ...
        if version.parse(browser_version) >= version.parse("115"):
            ...
            compatible_versions = [
                v for v in versions if short_version in v["version"]]
        ...
        raise Exception(
            f"No such driver version {browser_version} for {platform}")
